...
\begin{definition}\label{def:JointlyStronglyMeasurable}
  \leanok
  \lean{ProbabilityTheory.JointlyStronglyMeasurable}
We say that a stochastic process $X$ is jointly strongly measurable
if the joint map $(t, \omega) \mapsto X_t(\omega)$ is strongly measurable.
\end{definition}
...
We say that a stochastic process is integrable if it is jointly strongly measurable and for all $t$, $X_t$ is integrable.
...
\begin{definition}\label{def:locallyIntegrableSup}
  \uses{def:locally, ProbabilityTheory.HasIntegrableSup}
  \leanok
...
\begin{proof}
  \uses{lem:optionalSamplingSubmartingale}
...
\begin{proof}
  \uses{lem:Submartingale.classDL, lem:uniformIntegrable_of_tendstoInMeasure, lem:uniformIntegrableComp}
Assume that $X$ is uniformly integrable. We know from Lemma~\ref{lem:Submartingale.classDL} that $X$ is of class DL. Moreover, for any finite stopping time $\tau$, We have that $X_\tau = \lim_{n \to +\infty} X_{\tau \land n}$. Thanks to Lemma~\ref{lem:uniformIntegrable_of_tendstoInMeasure}, we deduce that $X$ is of class D.
...
\begin{lemma}\label{lem:isStable_jointlyStronglyMeasurable}
  \uses{def:stable, def:JointlyStronglyMeasurable}
  \leanok
  \lean{ProbabilityTheory.isStable_jointlyStronglyMeasurable}
The class of jointly strongly measurable processes is stable.
\end{lemma}
\begin{proof}
...
\begin{proof}\leanok
  \uses{lem:isStable_locally, lem:isStable_hasIntegrableSup}
This follows by combining Lemma~\ref{lem:isStable_locally} and Lemma~\ref{lem:isStable_hasIntegrableSup}.
\end{proof}
...
\begin{lemma}\label{lem:isStable_classDL}
  ...
  \leanok
  \lean{ProbabilityTheory.isStable_classDL}\leanok
...
Let $X$ be a stochastic process such that for all $t \in T$, $X^*_t := \sup_{s \le t} \|X_t\|$ is integrable. Then $X$ is of class DL.
\end{lemma}

\begin{proof}
  \uses{lem:uniformIntegrableDominatedSingleton}
Let $t \in T$. For every stopping time $\tau$ with $\tau \le t$, we have $\|X_\tau\| \le X^*_t$. Because by hypothesis $X^*_t$ is integrable, we deduce from Lemma~\ref{lem:uniformIntegrableDominatedSingleton} that $\{X_\tau \mid \tau \text{ is a stopping time with } \tau \le t\}$ is uniformly integrable. This proves that $X$ is of class DL.
\end{proof}


\begin{lemma}\label{lem:HasStronglyMeasurableSup_of_isCadlag}
  \uses{def:HasStronglyMeasurableSup, def:isCadlag, def:JointlyStronglyMeasurable}
  \leanok
  \lean{ProbabilityTheory.JointlyStronglyMeasurable.HasStronglyMeasurableSupProcess_of_isCadlag}
...
\begin{lemma}\label{lem:HasLocallyIntegrableSup.locally_classDL}
  \uses{def:locallyIntegrableSup, def:locally, def:classDL}
  \leanok
...
\begin{lemma}\label{lem:ClassDL.hasLocallyIntegrableSup}
  \uses{def:classD, def:locallyIntegrableSup}
  \leanok
...
\begin{proof}
  \leanok
  \uses{lem:local_induction, lem:ClassDL.hasLocallyIntegrableSup, lem:isStable_hasLocallyIntegrableSup}
Apply Lemma~\ref{lem:local_induction} using Lemma~\ref{lem:ClassDL.hasLocallyIntegrableSup} and Lemma~\ref{lem:isStable_hasLocallyIntegrableSup}.
\end{proof}
